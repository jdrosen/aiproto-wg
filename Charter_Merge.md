...
* Describe how the outcome of a confirmed agent operation is recorded as verifiable evidence: an operation record binding the operation, its disposition (performed, declined, or escalated to a human), and its inputs and outputs by digest, such that a relying party — including one in another administrative domain who trusts neither agent's operator — can verify what occurred. This composes with the confirmation attestation (which records what was approved before execution) and reuses existing IETF evidence and transparency mechanisms (e.g., RATS, SCITT) rather than redefining them.
